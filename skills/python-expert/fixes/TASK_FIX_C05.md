# TASK_FIX_C05 — Create `tests/test_celery_tasks.py`
**Severity:** 🔴 CRITICAL — Phase A Quality Gate requires moto + freezegun Celery task tests
...
No `moto`-based S3 tests exist. No `freezegun` usage. No QA task tests. No retry tests. The spec (TASK-030) requires full Celery task coverage with mocked AWS and time-frozen QA scans.
...
- `CELERY_TASK_ALWAYS_EAGER=True` is set in `config/settings/test.py` — tasks run **synchronously** in tests
- All `@mock_aws` tests must create the S3 bucket before uploading objects
- `freeze_time` must decorate individual test **methods**, not the class, when combined with `@mock_aws`
- `AWS_STORAGE_BUCKET_NAME` in test settings is `"test-airaad-bucket"` — use this exact name
- `ImportStatus` values: `QUEUED`, `PROCESSING`, `DONE`, `FAILED`
...
class TestProcessImportBatchTask:
    ...
    def test_s3_key_not_found_marks_batch_failed(self) -> None:
        ...
        # Do NOT upload anything — let S3 return NoSuchKey
        s3 = boto3.client("s3", region_name="us-east-1")
        bucket = getattr(settings, "AWS_STORAGE_BUCKET_NAME", "test-airaad-bucket")
        try:
            s3.create_bucket(Bucket=bucket)
    ...
    def test_nonexistent_batch_id_returns_cleanly(self) -> None:
        ...
        # Should not raise — task must handle missing batch gracefully
        try:
...
class TestWeeklyGpsDriftScan:
    ...
    def test_drift_over_threshold_flags_needs_review(self, vendor, field_agent_user) -> None:
        ...
        # Place vendor at (67.06, 24.82), visit GPS ~500m away
        vendor.gps_point = Point(67.06, 24.82, srid=4326)
        vendor.save(update_fields=["gps_point"])
...
class TestDailyDuplicateScan:
    ...
    def test_nearby_similar_name_flagged_as_duplicate(self, city, area) -> None:
        """Two vendors within 50m with >85% name similarity → both flagged NEEDS_REVIEW."""
        ...
        vendor_b = VendorFactory(
            ...
            gps_point=Point(67.0601, 24.8201, srid=4326),  # ~15m away
            qc_status=QCStatus.APPROVED,
        )
        ...
        assert vendor_a.qc_status == QCStatus.NEEDS_REVIEW
...
- **`@mock_aws` must wrap each test method** — not the class — when combined with `@freeze_time`
- **Bucket name** must match `settings.AWS_STORAGE_BUCKET_NAME` — read from settings, don't hardcode
- **`CELERY_TASK_ALWAYS_EAGER=True`** means tasks run synchronously — no `.delay()` needed in tests
- **`freeze_time`** must be applied to QA task tests that depend on `timezone.now()` for batch timing
- **No real AWS calls** — every test that touches S3 must use `@mock_aws`
- **`VendorFactory` GPS points** must be realistic Karachi coordinates (lon: ~67.0, lat: ~24.8)
- The `_make_s3_and_upload()` helper must handle `BucketAlreadyOwnedByYou` gracefully
...
```bash
cd airaad/backend
pytest tests/test_celery_tasks.py -v --no-header

# Expected output (all passing):
# TestProcessImportBatchTask::test_success_creates_vendors PASSED
# TestProcessImportBatchTask::test_idempotency_processing_status_skipped PASSED
# TestProcessImportBatchTask::test_error_log_capped_at_1000 PASSED
# TestProcessImportBatchTask::test_s3_key_not_found_marks_batch_failed PASSED
# TestWeeklyGpsDriftScan::test_drift_over_threshold_flags_needs_review PASSED
# TestDailyDuplicateScan::test_nearby_similar_name_flagged_as_duplicate PASSED
# ...

# Run with coverage to confirm task code is covered
pytest tests/test_celery_tasks.py --cov=apps/imports/tasks --cov=apps/qa/tasks --cov-report=term-missing
```
...
- **Correctness:** `_make_s3_and_upload()` helper handles `BucketAlreadyOwnedByYou` — no test pollution
- **Type Safety:** `-> None` on all test methods and helper; `from __future__ import annotations`
- **Performance:** `update_fields=["gps_point"]` on vendor saves — no full-model save overhead
- **Style:** Section comments per task; descriptive test names explain the scenario
